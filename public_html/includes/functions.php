...
function upload_image(array $file, string $targetDir): array {
    ...
    if (!isset($file['tmp_name']) || $file['error']!==UPLOAD_ERR_OK){ $r['error']='Upload failed'; return $r; }
    ...
    if ($file['size']>2*1024*1024){ $r['error']='File too large'; return $r; }
    $name=bin2hex(random_bytes(16)).'.'.$allowed[$mime]; $dest=rtrim($targetDir,'/\\').DIRECTORY_SEPARATOR.$name;
    if (!@move_uploaded_file($file['tmp_name'],$dest)){ $r['error']='Could not save file'; return $r; }
    $r['success']=true; $r['filename']=$name; return $r;
}
...
function send_email(string $to, string $subject, string $htmlBody, ?string $textBody=null): bool {
    ...
    if (!$phpMailerLoaded) {
        $maybe = __DIR__ . '/PHPMailer/src/PHPMailer.php';
    ...
    if ($phpMailerLoaded) {
        try {
            ...
            $mail->SMTPSecure=PHPMailer\PHPMailer\PHPMailer::ENCRYPTION_STARTTLS; $mail->Port=SMTP_PORT; $mail->CharSet='UTF-8';
            $mail->setFrom(SMTP_USER, SMTP_FROM_NAME); $mail->addAddress($to);
            $mail->isHTML(true); $mail->Subject=$subject; $mail->Body=$htmlBody; $mail->AltBody=$textBody ?? strip_tags($htmlBody);
            return $mail->send();
        } catch (Throwable $e) { log_error('send_email_phpmailer', $e); }
    }
    $headers=['MIME-Version: 1.0','Content-type: text/html; charset=UTF-8','From: '.SMTP_FROM_NAME.' <'.SMTP_USER.'>'];
...
function create_order(PDO $pdo, int $userId, array $cartItems, array $totals, array $customer): array {
    ...
    try {
        $stmt=$pdo->prepare("INSERT INTO orders (user_id,status,total_amount,tax_amount,shipping_amount,customer_name,customer_email,customer_phone,shipping_address) VALUES (?'=0', 'pending', ?, ?, ?, ?, ?, ?, ?)");
        // Some hosts may not like NULL via ? when strict; ensure null typed:
        $uid = $userId ?: null;
        $stmt=$pdo->prepare("INSERT INTO orders (user_id,status,total_amount,tax_amount,shipping_amount,customer_name,customer_email,customer_phone,shipping_address) VALUES (?,?,?,?,?,?,?,?,?)");
        $stmt->execute([$uid,'pending',$totals['total'],$totals['tax'],$totals['shipping'],$customer['name']??'',$customer['email']??'',$customer['phone']??'',$customer['address']??'']);
...
function create_razorpay_order(PDO $pdo, string $receipt, int $amountPaise, string $currency='INR'): array {
    $keyId=get_setting($pdo,'razorpay_key_id', RAZORPAY_KEY_ID);
    $secret=get_setting($pdo,'razorpay_key_secret', RAZORPAY_SECRET);
...
function webhook_remember(PDO $pdo, string $eventId, string $payload): void {
    try { $st=$pdo->prepare("INSERT INTO webhook_events (event_id, payload) VALUES (?, ?)"); $st->execute([$eventId,$payload]); }
